...
import Control.NewBookingCardController;
import Entity.BookingCard;

public class RegisterUI {
    private NewBookingCardController newBookingCardController;
    ...
    public void handleRegister() {
        newBookingCardController = new NewBookingCardController(new BookingCard());
        ...
        if (index == 1) {
            newBookingCardController.createPrepaidCards(list.get(0).toString(), (int) list.get(1),
                    list.get(2).toString(), list.get(3).toString(), list.get(4).toString(), (int) list.get(5));
        }else{
            newBookingCardController.createPostpaidCard(list.get(0).toString(), (int) list.get(1),
